...
```
kafka-console-consumer.sh --topic $(pulumi stack output --show-secrets topicName) --group my-group \
    --bootstrap-server $(pulumi stack output --show-secrets  bootstrapServers) \
...
```
kafka-console-consumer.sh --partition 0 --offset earliest --topic $(pulumi stack output --show-secrets topicName) \
    --bootstrap-server $(pulumi stack output --show-secrets  bootstrapServers) \
    --consumer.config <(pulumi stack output --show-secrets userProperties)  
```